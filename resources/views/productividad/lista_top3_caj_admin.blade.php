...
@section('content')
    <div class="container">
        ...
        @if ($top3 <3)
            <a href="/top3cajadmingen/" class="btn button-custom m-1">
                Generar top
            </a>
        @endif
            ...
            <tbody>
                @foreach ($lista_productividad as $index => $obj)
                    <tr>
                        ...
                        <td>{{ $obj->unidades }}</td>
                        <td>
                            <form action="/top3cajadmin/{{ $obj->id }}" class="form-del" method="POST">
                                @csrf
                                @method('DELETE')
                                <a href="/top3cajadminedit/{{ $obj->id }}" class="btn button-custom">Editar</a>
